...
public class DisplayHallsCommand extends Command {
    public DisplayHallsCommand() {
        super("displayhalls", "displayhalls ",
                "Displays all halls of the system", CommandType.EXTRA);
    }

    @Override
    public void execute(List<String> args, TicketSystem system) throws Exception {
       validateArgs(args,1,1);
        String result=system.displayHalls();
        if(result==null || result.trim().isEmpty())
        {
            System.out.println("No halls in the system");
            return;
        }
